    ...
    try {
        ...
        if (user) {
            ...
            if (isValidPass) {
                ...
                res.cookie('token', token, { httpOnly: true, secure: process.env.NODE_ENV === "production", maxAge: 7 * 24 * 3600000, sameSite: process.env.NODE_ENV === "production" ? "None" : "Lax", path: "/",});
...
export const createProfile = async (req, res) => {
  try {
    ...
    if (profileFile) {
      ...
      user.profile = uploadedProfile.secure_url;
    }
    ...
    for (const file of projectFiles) {
      ...
      newProjects.push({ index, imageUrl: uploaded.secure_url });
    }
...
export const logout = async (req, res) => {
    try {
        res.clearCookie('token', { httpOnly: true, secure: process.env.NODE_ENV === "production", maxAge: 7 * 24 * 3600000, sameSite: process.env.NODE_ENV === "production" ? "None" : "Lax", path: "/",});
